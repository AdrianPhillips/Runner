...
- (id)initWithSize:(CGSize)size playerWon:(BOOL)isWon
{
    ...
    if (self) {
        ...
        // 1
        SKLabelNode* wonGameLabel = [SKLabelNode labelNodeWithFontNamed:@"Arial"];
        wonGameLabel.fontSize = 42;
        wonGameLabel.position = CGPointMake(CGRectGetMidX(self.frame),350);
        if (isWon) {
            wonGameLabel.text = @"Game Won";
        }
        [self addChild:wonGameLabel];
    }
    return self;
}
