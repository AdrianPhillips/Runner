...
- (id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:_lives forKey:@"LivesSaved"];
...
- (void)initLivesLabel {
    ...
    _livesLabel.position = CGPointMake(150, self.frame.size.height - _livesLabel.frame.size.height * 1.5);
                               ...
                               usingBlock:^(SKNode *node, BOOL *stop) {
                                   ...
                                   if (CGRectIntersectsRect(_runner.frame, smallerFrame)) {
                                       if (_runnerState == incredible) {
                                           ...
                                           _livesLabel.text = [NSString stringWithFormat:@"%d Lives", _lives -1];
                                           [self restartGame];
                                       }
                                       else {
                                           [self gameOver];
...
- (void)restartGame {
    ...
    _isGameRestarted = YES;
    [_runner removeActionForKey:RUNNER_ANIMATION_KEY];
    RestartScene *restartScene = [[RestartScene alloc] initWithSize:self.size showRestartButton:YES];
    SKTransition *reveal = [SKTransition flipHorizontalWithDuration:2.0];
    [self.view presentScene:restartScene transition:reveal];
    _meter1 = [[NSUserDefaults standardUserDefaults] integerForKey:@"HighScoreSaved"];
    if (_meter > _meter1) {
        [[NSUserDefaults standardUserDefaults] setInteger:_meter forKey:@"HighScoreSaved"];
    }
